...
router.get('/app/getdiseases', (req, res) => {
    Disease.find({}, null, {sort: {name: 1}}).then((diseases) => {
        // Facade pattern -> make a simple JSON object, containing just the diseases names and StageOfDisease
        //                -> to easily communicate with the frontend
        var StageOfDiseaseJSON = {};

        if (_.isArray(diseases)) {
            for (var i = 0; i < diseases.length; ++i) {
                StageOfDiseaseJSON[diseases[i].name] = diseases[i].Stage;
            }
        }

        res.setHeader('Content-Type', 'application/json');
        res.status(200).send(JSON.stringify(StageOfDiseaseJSON));
    }).catch((err) => {
        console.log(err);
        res.status(404).send();
    });
...
router.post('/app/adddisease', (req, res) => {
    ...
    var diseaseStage = req.body.diseaseStage;

    // check that the name is a String and Stage is a Number
    if (_.isString(diseaseName) && !_.isNaN(diseaseStage)) {
        var disease = Disease({
