...
router.get('/app/registration', (req, res) => {
    res.render('registration', {pageTitle: ""});
});
...
router.post('/app/registration', (req, res) => {
    ...
    if (_.isEmpty(req.body.firstName) ||_.isEmpty(req.body.Nic) ||_.isEmpty(req.body.Address) || _.isEmpty(req.body.GuardianName) || _.isEmpty(req.body.hospitalNumber) || !isValidDate(dateOfBirth)) {
        if (_.isEmpty(req.body.firstName)) req.flash('error_msg', 'Please enter the first name.');
        if (_.isEmpty(req.body.Nic)) req.flash('error_msg', 'Please enter the first nic.');
        if (_.isEmpty(req.body.Address)) req.flash('error_msg', 'Please enter the Address.');
        if (_.isEmpty(req.body.GuardianName)) req.flash('error_msg', 'Please enter the guardian name.');
        ...
        var patient = Patient({
            ...
            GuardianName: _.capitalize(req.body.GuardianName),
            sex: sex,
            Nic: _.capitalize(req.body.Nic),
...
/*
    POST /app/updatepatient/:hospitalNumber -> update disease & Stage for patient
